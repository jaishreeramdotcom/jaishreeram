...
const COUNTER_ID = 1; // Using a single row for the global counter
...
export default function App() {
  ...
  // Fetch initial count from Supabase and setup real-time subscription
  useEffect(() => {
    let mounted = true;
      ...
      try {
        ...
        if (error) {
          ...
          if (error.code === 'PGRST116') {
            ...
            } else if (mounted && newData) {
              setCount(newData.value);
              setDisplayCount(newData.value);
            }
          }
        ...
        .on(
          ...
          (payload) => {
            if (mounted && payload.new && 'value' in payload.new) {
              setCount(payload.new.value as number);
            }
          }
        )
        .subscribe();
